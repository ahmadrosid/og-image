...
const LinearGradient = ({ option, children, width = 1080, height = 1080 }) => {
    const bgOption = [
        ...
        "flex items-center justify-center p-12 bg-gradient-to-r from-sky-500 to-cyan-500 relative",
    ...
    return (
        <div style={{ width: `${width}px`, height: `${height}px` }} className={bgOption[option] ? bgOption[option] : `flex items-center justify-center p-12 bg-white relative`}>
            {children}
        </div>
    )
}
